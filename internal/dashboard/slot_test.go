...
func TestNothingDrawsAUnitLine(t *testing.T) {
	...
	*/
	...
	plain := decode(t, Render(d, reading(), 0, nil, nil))
	...
	spelled := decode(t, Render(d, reading(), 0, nil, nil))
...
func TestAPairGradesOnItsFirstReading(t *testing.T) {
	...
	*/
	...
	require.Positive(t, pixels(decode(t, Render(first, hot, 0, nil, nil)), colCrit),
		"a hot first reading was not drawn as one")

	second := first
	second.Headline = Slot{Source: readings.CPULoad, Second: readings.Coolant}
	require.Zero(t, pixels(decode(t, Render(second, hot, 0, nil, nil)), colCrit),
...
func TestTheStackedRowsAreSetLikeATable(t *testing.T) {
	...
	*/
	...
	img := decode(t, Render(d, r, 0, nil, nil))
	rows := fit(d, Stacked)
...
func TestTheReadingsSizeMovesTheReadings(t *testing.T) {
	...
	for _, size := range []int{50, 100, 150} {
		...
		tall = append(tall, digitHeight(decode(t, Render(d, reading(), 0, nil, nil)),
