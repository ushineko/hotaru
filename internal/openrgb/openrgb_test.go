...
func TestADeviceCanAcceptAModeAndNotHonourIt(t *testing.T) {
	...
	require.NoError(t, server.SetMode(t.Context(), "Generic Strip", "Static", openrgb.Style{}))
	...
	require.NoError(t, server.SetMode(t.Context(), "Generic Strip", "Breathing", openrgb.Style{}))
	list, err = server.Devices(t.Context())
	require.NoError(t, err)
...
func TestAStoppedServerFailsEveryCallRatherThanSomeOfThem(t *testing.T) {
	...
	require.Error(t, err)
	require.Error(t, server.SetMode(t.Context(), "Generic Strip", "Static", openrgb.Style{}))
...
func TestWhatIsNotThereIsNamedRatherThanIgnored(t *testing.T) {
	...
	require.ErrorContains(t, server.SetMode(t.Context(), "Generic Strip", "Rainbow", openrgb.Style{}), `no mode called "Rainbow"`)
}
