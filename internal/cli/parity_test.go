...
func TestEveryRouteTheServiceServesIsReachableFromTheCommandLine(t *testing.T) {
	...
	for _, args := range [][]string{
		...
		{"readings"},
		{"dashboard", "list"},
		{"dashboard", "use", "coolant"},
		{"dashboard", "preview", "coolant", "--to", "/dev/null"},
		{"dashboard", "save", "parity", "--from", "testdata/dashboard.yml"},
		{"dashboard", "delete", "parity"},
		{"screen", "readout"},
		{"scene", "list"},
		{"scene", "write", "parity", "kraken=red"},
		{"scene", "save", "parity"},
		{"scene", "apply", "parity"},
		{"scene", "preview", "--detach", "kraken=red"},
		{"scene", "delete", "parity"},
		{"keys"},
