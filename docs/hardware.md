...
| Keychron K4 HE | OpenRGB | Advertises no Off mode, so `off` would resolve to Direct with black and kill the backlight. Coloured, never blanked. **Per-key colour takes hue but not value**: `#ffffff` and `#101010` are indistinguishable on the board and `#000000` is as lit as anything else, while a red row beside a green one draws correctly. Reproduces through OpenRGB's own command line, so it is not hotaru's. Firmware `v1.1.1 2025-06-17`. **And the lock keys are the firmware's**: Caps Lock and Num Lock are painted white while engaged, over whatever is written, and cannot be overridden — deliberately, since an indicator a host can switch off is one that will be off when it matters. Both findings are why spec 043 colours those keys rather than blanking them (#110) |
